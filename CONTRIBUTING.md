# Contributing to ep_search

Thanks for helping improve `ep_search` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_search` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_search from your local clone
pnpm run plugins i --path ../ep_search

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_search/` are picked up after a server restart.
...
* Etherpad version, ep_search version, browser + OS
* Steps to reproduce
